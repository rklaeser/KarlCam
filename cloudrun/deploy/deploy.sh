...
if [ ! -f "${SCRIPT_DIR}/deploy-infrastructure.sh" ]; then
    echo "❌ Missing deploy-infrastructure.sh in ${SCRIPT_DIR}"
    exit 1
fi
...
if [ "$#" -gt 0 ]; then
    case "$1" in
        ...
        collector)
            ...
            "${SCRIPT_DIR}/deploy-collector.sh"
            ;;
        api)
        ...
        frontend)
            ...
            ;;
        review-backend)
            echo "🔍 Deploying review backend only..."
            "${SCRIPT_DIR}/deploy-review-backend.sh"
            ;;
        review-frontend)
            echo "🎨 Deploying review frontend only..."
            "${SCRIPT_DIR}/deploy-review-frontend.sh"
            ;;
        review)
            echo "📊 Deploying review system (backend + frontend)..."
            "${SCRIPT_DIR}/deploy-review-backend.sh"
            echo ""
            "${SCRIPT_DIR}/deploy-review-frontend.sh"
            ;;
        all|"")
            ...
            ;;
        *)
            echo "Usage: $0 [infrastructure|collector|api|frontend|review|review-backend|review-frontend|all]"
            echo ""
            ...
            echo ""
            echo "Review System:"
            echo "  review         - Deploy both review backend and frontend"
            echo "  review-backend - Review backend API only"
            echo "  review-frontend- Review frontend UI only"
            echo ""
            ...
            echo "  $0 review       # Deploy review system"
            exit 1
            ;;
    esac
else
...
if [ "$1" == "all" ] || [ "$#" -eq 0 ]; then
    ...
    echo "  • Schedule collection: ./setup-scheduler.sh"
    echo ""
